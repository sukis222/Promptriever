...
- Изучение существующих моделей промпт ретриверов, тестирование, выбор подходящей, дообучение ее на русском языке

Почему возникла идея это протестировать?
- Мне интересно изучать NLP, на примере этого проекта я смогу лучше узнать методы обработки и векторизации текста, также адаптирвоть существующие модели под русский язык

Каковы критерии успеха с точки зрения исследования.
- Добиться хороших значений метрик и в целом рабочей модели ретривера, которая будет работать с текстом на русском языке. 

### 1.2 Ограничения и допущения

Так как модель будет на базе уже существующего Promptriever, в ней будут те же ограничения: 
- В сгенерированных инструкциях для обучения модели могут быть ошибки и некорректная информация (Общее ограничение языковых моделей)
- Предобученная модель на английском языке будет сохранять особенности языка, при дообучении могут возникунуть морфологические проблемы 
...
- Генерируем порядка 30 hard negatives под каждый запрос и 1 верный ответ
